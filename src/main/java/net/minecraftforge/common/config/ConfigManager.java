...
import my_life_is_bad.configurationsbackport.mixins.MixinEnumHolder;
...
public class ConfigManager
{
    ...
    public static void loadData(ASMDataTable data)
    {
        ...
        for (ASMData target : data.getAll(Config.class.getName()))
        {
            ...
            Config.Type type = tholder == null ? Config.Type.INSTANCE : Config.Type.valueOf(((MixinEnumHolder) tholder).getValue());
    ...
    {
        FMLLog.fine("Attempting to inject @Config classes into {} for type {}", modid, type);
        ...
        for (ASMData targ : map.get(type))
        {
            ...
            {
                FMLLog.severe("An error occurred trying to load a config for {} into {}", modid, targ.getClassName(), e);
